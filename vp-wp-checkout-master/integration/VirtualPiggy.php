...
class VirtualPiggy {
    ...
    private function getPaymentService() {
        return new VirtualPiggyPaymentService($this->config);
    }
    ...
    public function login($username, $password) {
        ...
        if ($userDTO->UserType == VirtualPiggy::USER_TYPE_PARENT) {
            ...
            $userDTO->parent = $this->getChildParent($user->Token);
        }
    ...
    public function getShippingDetails() {
        if ($this->isParent()) {
            $result = $this->getShippingDetailsBySelectedChild();
        } else {
    ...
    private function getCartDTOByWooCommerceOrder(WC_Order $order) {
        ...
        if(method_exists($order, 'get_total')){
            ...
            $cartDTO->Total = $order->get_order_total();
            $cartDTO->ShippmentTotal = $order->get_order_total();
        }
    ...
    private function getShippingAddressDTOByWooCommerceOrder(WC_Order $order) {
        ...
        $shippingAddressDTO->ParentName = $this->getShippingDetails()->ParentName;
    ...
    private function getChildParent($childToken) {
        return $this->getPaymentService()->GetParentAddress($childToken);
    }

    public function t() {
        pr($this->user->Token);
        return $this->getPaymentService()->GetChildAddress($this->user->Token);
    }
}